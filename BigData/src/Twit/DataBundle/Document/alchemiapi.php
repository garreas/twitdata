...
class alchemiapi {
    ...
    public function sentiment($flavor, $data, $options) {
        //Make sure this request supports the flavor
        if (is_array($this->_ENDPOINTS['sentiment']) && array_key_exists($flavor, $this->_ENDPOINTS['sentiment'])) {
    ...
    */
    ...
    private function analyze($endpoint, $params) {
        //Insert the base URL
        $url = $this->_BASE_URL . "/text/TextGetTextSentiment" . "?";

        //Add the API Key and set the output mode to JSON
        $params['apikey'] = '1f00b17bbd3c66ff251fe27b173f289b814cb341';
        $params['outputMode'] = 'json';

        //Create the HTTP header
        $header = array('http' => array('method' => 'POST','header'=>"Content-type: application/x-www-form-urlencoded\r\n", 'content'=>http_build_query($params)));
